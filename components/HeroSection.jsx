...
const HeroSection = () => {
  ...
  // --- GSAP Master Reveal Timeline ---
  useGSAP(() => {
    const tl = gsap.timeline({ defaults: { ease: 'power3.out' } });

    // Initial state setup
    gsap.set('.hero-anim', { y: 40, opacity: 0 });
    gsap.set('.hero-img', { x: 50, opacity: 0, scale: 0.9, rotation: -5 });

    tl.to('.hero-title', { y: 0, opacity: 1, duration: 1, delay: 0.2 })
      .to('.hero-subtitle', { y: 0, opacity: 1, duration: 0.8 }, "-=0.6")
      .to('.hero-desc', { y: 0, opacity: 1, duration: 0.8 }, "-=0.6")
      .to('.hero-buttons button', { y: 0, opacity: 1, duration: 0.6, stagger: 0.15, ease: 'back.out(1.5)' }, "-=0.6")
      .to('.hero-img', { x: 0, opacity: 1, scale: 1, rotation: 0, duration: 1.2, ease: 'expo.out' }, "-=1");
  ...
  return (
    <Section 
      id={SECTION_IDS.HOME} 
      className="group/section min-h-screen flex items-center pt-16 md:pt-0 bg-neutral-950 relative overflow-hidden" 
    >
      <HeroBackgroundSVG mousePos={mousePosition} />

      <div ref={spotlightRef} className="absolute inset-0 z-[5]">
        <div className={`hero-spotlight transition-opacity duration-700 ${spotlightVisible ? 'opacity-100' : 'opacity-0'}`}></div>
      </div>
      ...
      <div ref={sectionContentRef} className="container mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-12 relative z-10">
        <div className="md:w-3/5 text-center md:text-left">
          <h1 className="hero-anim hero-title text-4xl sm:text-5xl lg:text-6xl font-bold text-neutral-100 mb-4">
            Hello, I'm <span className="text-primary-light inline-block">Sarim Ali</span>
          </h1>
          
          <div className="hero-anim hero-subtitle h-10 sm:h-12 lg:h-14 mb-6 relative">
          ...
          <div className="hero-buttons flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
            <button
              ...
              className="hero-anim bg-primary hover:bg-primary-dark text-white font-semibold py-3 px-8 rounded-lg shadow-md btn-glow-primary"
            >
              View My Projects
            </button>
            <button
              onClick={() => navigateToSection(SECTION_IDS.ABOUT)}
              className="hero-anim bg-transparent hover:bg-neutral-700 text-neutral-100 font-semibold py-3 px-8 rounded-lg border-2 border-neutral-400 btn-glow-outline"
            >
              Learn More About Me
            </button>
          </div>
        ...
        <div className="md:w-2/5 flex justify-center perspective-[1000px]">
          <img
            src={PHOTO_URL}
            alt="Sarim Ali"
            className="hero-img rounded-full w-64 h-64 sm:w-80 sm:h-80 lg:w-96 lg:h-96 object-cover shadow-2xl border-4 border-primary/50 hover:border-primary transition-colors duration-500"
          />
        </div>
      </div>
    </Section>
